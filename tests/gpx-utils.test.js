...
describe("gpx-utils", () => {
  it("parseTrackFileName parses valid GPX names", () => {
  ...
  it("parseTrackFileName supports legacy GPX names without activity type", () => {
  ...
  it("parseTrackFileName returns null for invalid names", () => {
  ...
  it("parseTrackFileName falls back to legacy parsing for unknown activity type", () => {
  ...
  it("parseTrackFileName keeps multi-hyphen route name after activity type", () => {
  ...
  it("toTitleCase normalizes separators and capitalizes words", () => {
    assert.equal(toTitleCase("beskid_wyspowy-trasa  dluga"), "Beskid Wyspowy Trasa Dluga");
  });

  it("filterTracksByName is case-insensitive, trims query, and matches date", () => {
    const tracks = [
